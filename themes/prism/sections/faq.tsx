...
export function PrismFaq({
  ...
  return (
    <PrismSection size="lg" id="faq">
      <PrismHeading
        align="center"
        sticker={<span className="prism-sticker">{isAr ? "أسئلة شائعة" : "FAQ"}</span>}
      ...
      <div className="max-w-3xl mx-auto mt-14 space-y-3">
        {items.map((it, i) => {
          ...
          return (
            <div
              key={i}
              className={`prism-card prism-fade-up ${isOpen ? "border-fuchsia-400/40" : ""}`}
              style={{ ["--prism-delay" as string]: `${i * 50}ms` }}
            >
              <button
                type="button"
              ...
              >
                <span className="prism-mono text-[11px] text-cyan-300 w-8 shrink-0">
                  Q{String(i + 1).padStart(2, "0")}
                </span>
                <span className="flex-1 font-bold text-white text-base md:text-lg">{it.q}</span>
                <span className="grid place-items-center h-9 w-9 rounded-full border-2 border-white shrink-0 text-white">
              ...
              >
                <div className="overflow-hidden">
                  <p className="px-5 pb-5 ps-[3.6rem] text-white/75 leading-relaxed">{it.a}</p>
                </div>
              </div>
            </div>
          );
